...
· 上传(拖拽文件·文件夹 / 多选 / 并发分片 / 进度)· 下载(流式 / 批量 / 断点续传)· 重命名 / 复制·移动(级联选择器)
· 跨账号·跨云迁移 · 文件夹级递归(下载 / 迁移 / 删除整个目录)· 完整性校验(MD5 对比 ETag)· 桶内递归搜索 · 分享(预签名链接)· 新建文件夹 · 删除(单个 / 批量)· 传输面板(并发 / 全局限速 / 重试)· 设置 · 明暗主题 · 快捷键 · 右键菜单。
