...
· 跨账号·跨云迁移 · 跨区域桶自动路由(每个桶连它自己的区域)· 文件夹级递归(下载 / 迁移 / 复制·移动·重命名 / 删除整个目录)· 完整性校验(MD5 对比 ETag)· 存储类型转换 / 归档取回(单个 / 多选 / 整个文件夹)· 桶内递归搜索(名字 / 大小 / 类型过滤 · 结果可多选批量转档·取回·删除·下载)· 文件夹 / Bucket 统计(对象数 / 总大小 / 按存储类型分布)· 分享 / 上传链接(预签名下载·上传)· 对象公开读 / 私有 + 永久公共直链(可配自定义域名 / CDN)· 新建 / 删除 Bucket · 清理未完成的分片上传(回收残留分片费用)· 新建文件夹 · 删除(单个 / 批量)· 传输面板(并发 / 全局限速 / 速度·ETA / 取消·续传 / 重启恢复 / 重试含迁移)· 设置 · 中英双语 · 明暗主题 · 快捷键 · 右键菜单。
