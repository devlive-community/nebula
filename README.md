...
· 跨账号·跨云迁移 · 文件夹级递归(下载 / 迁移 / 删除整个目录)· 完整性校验(MD5 对比 ETag)· 存储类型转换 / 归档取回(单个 / 多选 / 整个文件夹)· 桶内递归搜索(名字 / 大小 / 类型过滤)· 文件夹 / Bucket 统计(对象数 / 总大小)· 分享 / 上传链接(预签名下载·上传)· 新建 / 删除 Bucket · 新建文件夹 · 删除(单个 / 批量)· 传输面板(并发 / 全局限速 / 速度·ETA / 取消·续传 / 重启恢复 / 重试含迁移)· 设置 · 中英双语 · 明暗主题 · 快捷键 · 右键菜单。
