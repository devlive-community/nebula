...
- **Backup & sync** — pair a local folder with a cloud prefix and back up / restore / two-way sync, with a per-file diff preview, glob exclude rules (e.g. `.DS_Store`, `node_modules/**`), progress and cancel; reuses resumable transfer, skip-unchanged and rate limiting.
