...
**A cross-platform desktop manager for multi-cloud object storage** — manage Aliyun OSS, Tencent COS, Huawei OBS, AWS S3, Cloudflare R2, Qiniu Kodo, MinIO, Backblaze B2, Wasabi, DigitalOcean Spaces, Scaleway Object Storage, UCloud US3, JD Cloud OSS and UPYUN from a single native app, just like a local file manager: browse, upload, download, share and migrate across clouds, all in one place.
...
| UCloud US3 | ✅ | S3-compatible, SigV4-only (custom region-from-endpoint client) |
| JD Cloud OSS | ✅ | S3-compatible (shares `s3-core`) |
| UPYUN | ⚠️ unverified region | S3-compatible; SigV4 `region` value is a guess (`us-east-1`), not officially confirmed |
...
- **Backup & sync** — pair a local folder with a cloud prefix and back up / restore / two-way sync, with a per-file diff preview, glob exclude rules (e.g. `.DS_Store`, `node_modules/**`), progress and cancel; save named jobs and run them on a schedule (every 15 min … daily); a real local file-system watcher triggers a job sooner when the local folder changes instead of waiting for the next tick, with interval polling as a fallback; reuses resumable transfer, skip-unchanged and rate limiting.
...
- **Share & publish** — presigned download / upload links; make an object public and get a **permanent direct link**, optionally through a custom domain / CDN configured per account; AWS S3 / Huawei OBS additionally support **fine-grained access** — grant a specific account ID access to an object instead of only the public/private toggle (other vendors don't support account-ID grants and aren't pretended to).
...
- **Storage hygiene** — clean up orphaned incomplete multipart uploads to reclaim silently-billed storage; find duplicate objects (grouped by size + ETag) and delete redundant copies keeping one; rank the largest files in a folder to see what to clean; transition tiers or restore archives for a single object or a whole folder; per-bucket governance panels for lifecycle rules (auto-expire / auto-tier by prefix and age), versioning (enable / suspend, browse and restore or permanently delete old versions), CORS rules and static website hosting — each exposed only where the connected provider genuinely supports it.
