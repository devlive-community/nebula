...
- **备份与同步** —— 把本地目录与云端前缀对起来,做备份 / 还原 / 双向同步;逐文件 diff 预览、glob 排除规则(如 `.DS_Store`、`node_modules/**`)、实时进度与取消;可**保存为命名任务并定时运行**(每 15 分钟…每天);复用续传、秒传跳过与限速基建。
...
- **存储治理** —— 一键清理桶内残留的未完成分片上传,回收白白计费的存储;查找重复对象(按大小 + ETag 分组)、保留一份删除多余副本;按大小给文件夹里的对象排行,一眼看到最占空间的;单个对象或整个文件夹都能转换存储层、取回归档。
