...
- **浏览与预览** —— 目录逐层展开、列表 / 网格视图、排序过滤;**Rust 加速的图片浏览器**(在独立窗口打开单张图;后端解码 / 缩放到视口大小 / 按 ETag 磁盘缓存,前端只拿小图,支持缩放·平移·旋转·EXIF);视频 / 音频 / PDF / 文本代码预览;对象详情含存储类型、真实 Content-Type(可改)与可读写的对象标签;文件夹 / Bucket 大小统计与按存储类型的分布。
