...
**跨平台桌面端的多云对象存储管理器** —— 在一个原生应用里,像用本地文件管理器一样管理阿里云 OSS、腾讯云 COS、华为云 OBS、AWS S3、Cloudflare R2、七牛云 Kodo、MinIO、Backblaze B2、Wasabi、DigitalOcean Spaces、Scaleway Object Storage、UCloud US3、京东云 OSS、又拍云:浏览、上传下载、分享、跨云迁移,一站搞定。
...
| 京东云 OSS | ✅ | S3 兼容(共用 `s3-core`) |
| 又拍云 | ⚠️ region 未经证实 | S3 兼容;SigV4 的 `region` 值是推断(`us-east-1`),未经官方确认 |

> 新增一家厂商 = 写一个 `<vendor>` SDK(S3 兼容的可直接复用 `s3-core`)+ 一个 provider 适配层,在 `app-core` 注册即可,**App 界面无需改动**。

## 功能

- **浏览与预览** —— 目录逐层展开、列表 / 网格视图、排序过滤;**Rust 加速的图片浏览器 / 编辑器**(在独立窗口打开单张图;后端解码 / 缩放 / 按 ETag 磁盘缓存,前端只拿小图,支持缩放·平移·旋转·EXIF;编辑支持裁剪 / 旋转 / 拉直 / 翻转 / 调整尺寸 / 灰度 / 反相 / 画笔 / 矩形 / 箭头 / 文字(可描边)/ 序号标记、吸管取色 / 马赛克打码 / 魔术棒 · 橡皮擦局部擦除 / 亮度 / 对比度 / 饱和度 / 色温 / 锐化 / 模糊 / 色相,撤销重做,外加**可下载的 AI 去背景插件**(本地 ONNX Runtime + u2netp,全程在本机推理、不上传任何图片),可选 JPEG / PNG / WebP 格式与画质,实时预览后一键存回云端(覆盖或另存为新对象)或下载到本地);**独立的 PDF 页面编辑器**(翻页 / 缩放阅读,删页 / 拖拽重排 / 旋转 / 合并另一个 PDF / 提取页 / 加页码 / 加文字水印 / 提取文本(复制或导出 txt)/ 压缩瘦身(省存储)——后端用纯 Rust lopdf 解析与重新组装,存回云端或下载到本地);视频 / 音频 / 文本代码预览;对象详情含存储类型、真实 Content-Type(可改)与可读写的对象标签;文件夹 / Bucket 大小统计与按存储类型的分布。
- **上传与下载** —— 拖拽文件 / 文件夹、多选上传;大文件并发分片、断点续传;内容未变的文件自动秒传跳过;流式下载不占内存、整目录递归下载。
- **备份与同步** —— 把本地目录与云端前缀对起来,做备份 / 还原 / 双向同步;逐文件 diff 预览、glob 排除规则(如 `.DS_Store`、`node_modules/**`)、实时进度与取消;可**保存为命名任务并定时运行**(每 15 分钟…每天);本地目录一有变化就靠真正的文件系统监听提速触发,不用等到下一个整点,按间隔轮询继续作为兜底;复用续传、秒传跳过与限速基建。
- **迁移与整理** —— 对象或整个文件夹跨账号、跨云迁移(同账号服务端复制,跨账号自动中转);同账号内复制 / 移动 / 重命名到任意目录;多选对象**批量重命名**(加前缀 / 后缀 / 查找替换,先预览后执行)。
- **分享与公开** —— 生成预签名下载 / 上传临时链接;把对象设为公开读并拿到**永久公共直链**,还能给账号配自定义域名 / CDN,直链走你自己的域名;AWS S3 / 华为云 OBS 额外支持**细粒度授权**——按具体账号 ID 分享给某个协作者,而不只是公开/私有二选一(其它厂商官方不支持按账号 ID 授权,不会假装能用)。
- **搜索与批量** —— 桶内递归搜索,支持按**文件名**(名字 / 大小 / 类型过滤)或按**内容**(在文本与 PDF 文件里搜,带命中片段);结果可多选批量转档 / 取回 / 删除 / 下载;跨区域的桶自动路由到各自的区域。
- **导航与效率** —— **Cmd/Ctrl+K 命令面板**(输入即跳转账号 / 收藏或执行命令)、**收藏夹**、**最近访问**;中英双语、明暗主题、快捷键、右键菜单。
- **传输管理** —— 并发控制、全局限速、速度 / ETA、取消 / 续传、重启恢复、一键重试全部失败。
- **存储治理** —— 一键清理桶内残留的未完成分片上传,回收白白计费的存储;查找重复对象(按大小 + ETag 分组)、保留一份删除多余副本;按大小给文件夹里的对象排行,一眼看到最占空间的;单个对象或整个文件夹都能转换存储层、取回归档;桶级治理面板覆盖生命周期规则(按前缀 / 天数自动过期或转归档)、版本控制(启用 / 暂停,浏览与恢复或永久删除旧版本)、CORS 规则与静态网站托管——按每家云真实支持的能力开放,不支持的不会假装能用。
...
│  ├─ backblaze-b2/             Backblaze B2 SDK(S3 兼容,s3-core 门面)
...
│  ├─ jdcloud-oss/              京东云 OSS SDK(S3 兼容,s3-core 门面)
│  ├─ upyun-uss/                又拍云 SDK(S3 兼容,region 硬编码为未经证实的推断值)
