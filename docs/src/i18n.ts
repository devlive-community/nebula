  ...
  zh: {
    ...
    features: {
      ...
      items: {
        ...
        upload: {title: '上传无忧', desc: '拖拽文件或整个文件夹、多选上传;大文件并发分片、断点续传,内容未变的文件自动秒传跳过,可随时取消,进度、并发、失败重试(可一键重试全部失败)尽在掌握;还能一键清理桶里残留的未完成分片上传、查找重复对象删掉多余副本,回收白白计费的存储。'},
