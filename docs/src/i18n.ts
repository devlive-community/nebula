...
const messages = {
  zh: {
    ...
    hero: {
      ...
      title1: '一个界面,管理你的',
      title2: '多云对象存储',
      subtitle: '在一个原生桌面应用里管理阿里云 OSS、腾讯云 COS、华为云 OBS、AWS S3、Cloudflare R2、七牛 Kodo、MinIO、Backblaze B2、Wasabi、DigitalOcean Spaces、Scaleway Object Storage、UCloud US3、京东云 OSS、又拍云 —— 浏览、上传下载、分享、跨云迁移,一站搞定。轻量、快速,macOS / Windows / Linux 原生三端。',
      download: '立即下载',
      github: '在 GitHub 查看'
    },
    preview: {
    ...
    features: {
      ...
      title: '一个应用,搞定云上文件',
      items: {
        multicloud: {title: '多云统一', desc: '一套界面管理多云对象存储,连 Bucket 的新建 / 删除都统一;跨区域的桶自动路由到各自的区域,不用手动切;桶级治理也在同一套界面里——生命周期规则(按前缀 / 天数自动过期或转归档)、版本控制(启用 / 暂停,浏览与恢复或永久删除旧版本)、CORS 规则与静态网站托管,按各家云的真实支持情况开放;新增厂商实现一个接口即可,界面零改动。'},
        browse: {title: '高效浏览', desc: '目录逐层展开、列表 / 网格视图、排序过滤、常去位置一键收藏跳转、Cmd/Ctrl+K 命令面板秒跳账号与收藏、直接执行刷新 / 上传 / 新建等命令(空输入先列最近访问)、Rust 加速的图片浏览器 / 编辑器(独立窗口打开单张,后端解码缩放 + 磁盘缓存;缩放 / 平移 / 旋转 / EXIF;编辑裁剪 / 旋转 / 翻转 / 灰度 / 反相 / 亮度对比度 / 标注 / 马赛克 / 魔术棒 · 橡皮擦局部擦除,还有可下载的 AI 去背景插件(本地推理、不上传),实时预览并存回云端)、独立的 PDF 页面编辑器(翻页缩放阅读 + 删页 / 重排 / 旋转 / 合并 / 提取 / 加页码 / 加水印 / 提取文本 / 压缩瘦身,后端 lopdf 组装)、视频 / 音频 / 文本代码预览、按名或按内容(文本 / PDF)搜索、文件夹 / Bucket 大小统计与存储类型分布;对象详情含存储类型、真实 Content-Type(可改)与可读写的对象标签,单个对象或整个文件夹都能转换存储层、取回归档。'},
        upload: {title: '上传无忧', desc: '拖拽文件或整个文件夹、多选上传;大文件并发分片、断点续传,内容未变的文件自动秒传跳过,可随时取消,进度、并发、失败重试(可一键重试全部失败)尽在掌握;还能一键清理桶里残留的未完成分片上传、查找重复对象删掉多余副本、按大小排行看谁最占空间,回收白白计费的存储。'},
        download: {title: '流式下载', desc: '边下边写,大文件不占内存;支持断点续传、整文件夹递归下载,本地已有且一致的文件自动跳过,每个任务独立进度(含速度与剩余时间),可设全局带宽限速。'},
        sync: {title: '备份与同步', desc: '把本地目录与云端前缀对起来,做备份 / 还原 / 双向同步;先给逐文件 diff 预览(将上传 / 下载 / 删除哪些、共需传多少),确认再执行;支持 glob 排除规则(如 .DS_Store、node_modules/**)、实时进度与随时取消,可保存为命名任务并定时运行;本地目录一有变化就借助文件系统监听提速触发,不用等到下一个整点,按间隔轮询继续作为兜底,并复用续传、秒传跳过与限速。'},
  ...
  en: {
    ...
    features: {
      ...
      items: {
        multicloud: {title: 'Multi-cloud', desc: 'Manage object storage from one UI, down to creating and deleting buckets; buckets in other regions are routed to their own region automatically; bucket-level governance lives in the same UI too — lifecycle rules (auto-expire / auto-tier by prefix and age), versioning (enable / suspend, browse and restore or permanently delete old versions), CORS rules and static website hosting, exposed per each provider\'s real support; add a vendor by implementing a single trait — no UI changes.'},
        browse: {title: 'Fast browsing', desc: 'Drill into folders, list / grid views, sort and filter, bookmark and jump to frequent locations, a Cmd/Ctrl+K command palette to jump to any account or bookmark and run commands (recent locations listed first), a Rust-accelerated image viewer / editor that opens each image in its own window (backend decode + downscale + disk cache; zoom / pan / rotate / EXIF; edit with crop / rotate / flip / grayscale / invert / brightness / contrast / annotate / mosaic / magic-wand & eraser local removal plus a downloadable on-device AI background-removal plugin, and save back to the cloud), a standalone PDF page editor (read with pagination / zoom plus delete / reorder / rotate / merge / extract / page numbers / watermark / text extraction / compression, reassembled by lopdf on the backend), image / video / audio / text-code previews, search by name or by content (text / PDF),folder / bucket size stats and a storage-class breakdown; object details incl. storage class, the real Content-Type (editable) and read/write object tags, with tier transition and archive restore for a single object or a whole folder.'},
        upload: {title: 'Effortless upload', desc: 'Drag files or whole folders, multi-select; large files upload in concurrent chunks, resume after interruption, unchanged files are skipped instantly, cancel anytime, with progress and retries (retry all failed at once); and clean up orphaned incomplete multipart uploads find duplicate objects or rank the largest files to reclaim storage you are silently billed for.'},
        ...
        sync: {title: 'Backup & sync', desc: 'Pair a local folder with a cloud prefix and back up / restore / two-way sync. See a per-file diff preview (what will upload / download / delete and how much to transfer) before running; glob exclude rules (e.g. .DS_Store, node_modules/**), live progress and cancel, save named jobs and run them on a schedule; a local file system watcher triggers jobs sooner when the local folder changes instead of waiting for the next tick, with interval polling as a fallback, reusing resumable transfer, skip-unchanged and rate limiting.'},
