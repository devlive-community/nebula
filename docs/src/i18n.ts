  ...
  zh: {
    ...
    features: {
      ...
      items: {
        ...
        upload: {title: '上传无忧', desc: '拖拽文件或整个文件夹、多选上传;大文件自动分片,进度、并发、失败重试尽在掌握。'},
        download: {title: '流式下载', desc: '边下边写,大文件不占内存;批量下载到指定目录,每个任务独立进度。'},
  ...
  en: {
    ...
    features: {
      ...
      items: {
        ...
        upload: {title: 'Effortless upload', desc: 'Drag files or whole folders, multi-select; large files auto-chunk with progress, concurrency and retries.'},
        download: {title: 'Streaming download', desc: 'Stream to disk without buffering huge files; batch download to a folder with per-task progress.'},
