  ...
  zh: {
    ...
    features: {
      ...
      items: {
        ...
        browse: {title: '高效浏览', desc: '目录逐层展开、列表 / 网格视图、排序过滤、图片 / 视频 / 音频 / PDF / 文本代码预览、文件夹 / Bucket 大小统计与存储类型分布;对象详情含存储类型、真实 Content-Type(可改)与可读写的对象标签,单个对象或整个文件夹都能转换存储层、取回归档。'},
  ...
  en: {
    ...
    features: {
      ...
      items: {
        ...
        browse: {title: 'Fast browsing', desc: 'Drill into folders, list / grid views, sort and filter, image / video / audio / PDF / text-code previews, folder / bucket size stats and a storage-class breakdown; object details incl. storage class, the real Content-Type (editable) and read/write object tags, with tier transition and archive restore for a single object or a whole folder.'},
