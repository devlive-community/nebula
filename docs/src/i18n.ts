  ...
  zh: {
    ...
    features: {
      ...
      items: {
        ...
        browse: {title: '高效浏览', desc: '目录逐层展开、列表 / 网格视图、排序过滤、图片 / 视频 / 音频 / PDF / 文本代码预览、文件夹 / Bucket 大小统计;对象详情含存储类型、真实 Content-Type(可改)与可读写的对象标签,单个对象或整个文件夹都能转换存储层、取回归档。'},
        upload: {title: '上传无忧', desc: '拖拽文件或整个文件夹、多选上传;大文件并发分片、断点续传,内容未变的文件自动秒传跳过,可随时取消,进度、并发、失败重试尽在掌握。'},
        download: {title: '流式下载', desc: '边下边写,大文件不占内存;支持断点续传、整文件夹递归下载,每个任务独立进度(含速度与剩余时间),可设全局带宽限速。'},
        migrate: {title: '跨云迁移', desc: '把对象或整个文件夹从一个账号搬到另一个账号,任意云到任意云;同账号走服务端复制,跨账号自动中转;同账号内还能整目录移动 / 重命名。'},
        integrity: {title: '完整性校验', desc: '下载内容算 MD5 与远端 ETag 比对,一键确认文件是否在传输中损坏;逻辑只依赖 ETag,对每家云通用。'},
        share: {title: '一键分享', desc: '为对象生成预签名临时链接,有效期可配置,复制即分享;还能生成预签名上传链接,别人无需密钥凭链接直接 PUT 上传。'},
  ...
  en: {
    ...
    features: {
      ...
      items: {
        ...
        browse: {title: 'Fast browsing', desc: 'Drill into folders, list / grid views, sort and filter, image / video / audio / PDF / text-code previews, folder / bucket size stats; object details incl. storage class, the real Content-Type (editable) and read/write object tags, with tier transition and archive restore for a single object or a whole folder.'},
        upload: {title: 'Effortless upload', desc: 'Drag files or whole folders, multi-select; large files upload in concurrent chunks, resume after interruption, unchanged files are skipped instantly, cancel anytime, with progress and retries.'},
        ...
        migrate: {title: 'Cross-cloud migration', desc: 'Move objects or entire folders from one account to another, any cloud to any cloud; same-account uses server-side copy, cross-account relays automatically, and a whole folder can be moved / renamed within an account.'},
        ...
        share: {title: 'One-click share', desc: 'Generate presigned temporary links with a configurable expiry — copy and share; also generate presigned upload links so others can PUT a file with no credentials.'},
