<template>
  <div class="max-w-6xl mx-auto px-5 py-16">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
      <RouterLink v-for="r in pagedReleases" :key="r.version" :to="`/release/${r.version}`"
...
const PAGE_SIZE = 9
const page = ref(1)
const totalPages = computed(() => Math.max(1, Math.ceil(releases.length / PAGE_SIZE)))
const pagedReleases = computed(() => releases.slice((page.value - 1) * PAGE_SIZE, page.value * PAGE_SIZE))
