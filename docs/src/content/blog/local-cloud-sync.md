...
逻辑就是对两侧相对路径的**并集**逐个判断:

- 两侧都有 → 相同则 `Skip`,不同则按模式方向 `Upload` / `Download`;
- 只在本地 → 备份 / 双向 `Upload`;还原模式下若开了「删多余」则 `DeleteLocal`;
- 只在云端 → 还原 / 双向 `Download`;备份模式下若开了「删多余」则 `DeleteRemote`。

纯函数的好处是**可以彻底单测**:各模式、删除多余、双向双推、汇总统计,全部用内存里的假清单跑,不用起 GUI、不用连云。这也是 Nebula 一贯的做法——把「难的判断」从「脏的 IO」里剥出来。
