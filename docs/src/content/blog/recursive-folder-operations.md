...
整个文件夹之外,更常见的是「勾中一批文件,一起做点什么」。所以选择模式的动作栏把一批批量操作摆在一起:批量**移动 / 复制**到某个目录、批量打**标签**、批量设**公开 / 私有**、批量转存储类型 / 取回归档 / 删除。它们的实现都一样朴素——对选中项**逐个**调已有的单对象操作(复制 / 改名 / 设标签 / 设 ACL),带统一的进度与取消,单个失败记下继续。

以批量移动 / 复制为例:在目录选择器里挑一个目标目录,每个选中对象就落到 `目标目录/原文件名`;移动是「服务端复制到新路径 + 删原对象」,复制则只复制。没有新机制,只是把单对象能力套了个循环——但对「整理一批散落的文件」来说,一次操作胜过点几十次。
