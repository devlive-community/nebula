...
这篇写完之后过了一阵子才发现更根本的问题:**上传路径压根没给 Content-Type 赋过默认值**,一直原样传 `None` 到 provider,导致新传上去的图片、视频、PDF 大概率被对象存储回退成 `application/octet-stream`,预签名链接打开变成下载而不是预览——本文说的"点笔手动改"其实是在**补救**一个上传时就该做对的事。

修法是在上传的核心路径(`upload_resumable_parted`)里,调用方没显式传类型时按远端 key 的扩展名兜底推断:

```rust
let content_type = content_type.or_else(|| guess_content_type(remote_path));
```

`guess_content_type` 是一张纯扩展名 → MIME 的查表,覆盖常见图片 / 视频 / 音频 / 文档格式,查不到就还是 `None`(行为不变)。前端"修改内容类型"的输入框也顺带把默认值从空字符串改成同一套推断结果,不再是空着等用户自己想。手动改依然保留——推断只能按扩展名猜,猜不对或者对象本来就传错了,还是得靠这颗笔图标。
