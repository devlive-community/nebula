...
date: 2026-07-27
author: Nebula Team
description: 同一份文件在云上存了好几份,是悄悄花钱的常见来源。这篇讲 Nebula 怎么不下载文件、只靠列举时拿到的 ETag + 大小当「内容指纹」来找重复,为什么这个判据是保守可靠的,分组逻辑为什么写成纯函数,删除时「每组默认保留一份」的安全设计,以及配套的大文件排行。
...
这个功能实现上更简单——列举本来就返回了每个对象的大小,排序取 Top N 是个纯函数(降序、并列按路径稳定、`top` 超过文件数也安全),同样脱离云端单测。它和重复查找共用同一套「递归列举 + 纯逻辑」的骨架:找重复是按内容签名**分组**,找大文件是按大小**排序**,都不下载文件、都把判断做成可测的纯函数。清理云存储,先看清「重复的」和「最大的」这两类,往往就能回收掉大头。
