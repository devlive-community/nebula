...
date: 2026-07-10
author: Nebula Team
description: 接了好几家云,却只能一家一家单独用,数据搬家还得先下到本地再传上去——那"多云"就只是个空壳。这篇讲我们怎么加"A 云 → B 云"的一键迁移,以及一个绕不过的坑:服务端复制根本跨不了账号。
tags: ['开发', '多云', '对象存储', '架构']
---

Nebula 已经接入了多家对象存储:国内的阿里云 OSS / 腾讯云 COS / 华为云 OBS,国际的 AWS S3 / Cloudflare R2,以及自建的 MinIO 等。但有个尴尬的事实:在加这个功能之前,"多云"其实是个**空壳**——你能在一个 App 里切换好几家云,却只能一家一家单独用。想把阿里云某个桶里的文件搬到 AWS?先下载到本地,再手动上传上去。多云工具最该解决的"跨云搬家",反而得靠人肉。
...
`read` 内部是 OSS 的 V2 签名还是 AWS 的 SigV4,`write` 内部要不要分片、怎么分片,全被各自的 provider 适配层吃掉了。迁移逻辑站在 `StorageProvider` 这层统一抽象之上,于是 **"从任意一家云搬到任意一家云"的所有组合,一行都不用单独写**,天然全覆盖:接入 N 家云,就有 N × N 种搬运方向,全部免费。以后再接一家新云,它和已有各家之间的双向迁移**自动就通了**——不需要动这里一行代码。
