//! 把 [`aws_s3::S3Client`] 适配成统一的 [`StorageProvider`]。
//!
//! App 私有适配层(不发布)。AWS S3 走 S3 REST API + SigV4,路径遵循 `bucket/key` 约定,
//! 与阿里云 / 华为云 / 七牛云适配层同构。
...
use aws_s3::{AclGrant, ListEntry, S3Client, S3Error};
use nebula_provider::{
...
fn grant_from_sdk(g: AclGrant) -> Option<Grant> {
    Some(Grant {
...
fn grant_to_sdk(g: &Grant) -> AclGrant {
    AclGrant {
...
/// AWS S3 的 provider 适配器。一个实例 = 一个账号。
...
/// 把 S3 错误映射到统一 provider 错误。
/// 把 SDK 的生命周期规则映射到统一 provider 模型(字段一一对应)。
fn rule_from_sdk(r: aws_s3::LifecycleRule) -> LifecycleRule {
    LifecycleRule {
        id: r.id,
        prefix: r.prefix,
        enabled: r.enabled,
...
fn version_from_sdk(v: aws_s3::ObjectVersion) -> ObjectVersion {
    ObjectVersion {
...
fn rule_to_sdk(r: &LifecycleRule) -> aws_s3::LifecycleRule {
...
impl StorageProvider for AwsProvider {
    ...
    fn capabilities(&self) -> Capabilities {
        Capabilities {
            ...
            object_acl: true,
            // AWS S3 官方支持按 canonical user ID 授权对象级 ACL(GetObjectAcl/PutObjectAcl)。
...
mod tests {
    ...
    fn provider() -> AwsProvider {
        AwsProvider::new("test", "ak", "sk", "s3.us-west-2.amazonaws.com")
    }
