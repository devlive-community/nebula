//! 用真实 AWS S3 账号做一次端到端冒烟:put → head → list → get → delete → multipart。
//!
//! ```bash
//! export AWS_ACCESS_KEY=你的AK
//! export AWS_SECRET_KEY=你的SK
//! export AWS_ENDPOINT=s3.us-east-1.amazonaws.com   # 你 bucket 所在区域
//! export AWS_BUCKET=你的bucket名
//! cargo run -p aws-s3 --example aws_smoke
//! ```
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let client = S3Client::new(ak, sk, endpoint);
    let key = "nebula-smoke-test.txt";
    let content = b"hello from nebula aws-s3 smoke test";
...

    {
        ...
        println!("[0/5] list_buckets  ✅ 账号下 {} 个 bucket", names.len());
    }
    ...
    let part_size = aws_s3::MIN_PART_SIZE; // 5 MiB
