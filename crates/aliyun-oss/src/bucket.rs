...
impl OssClient {
    ...
    pub async fn get_bucket_cors(&self, bucket: &str) -> Result<Vec<CorsRule>> {
        ...
        let resp = match check_status(self.http().execute(request).await?).await {
            Ok(resp) => resp,
            Err(OssError::Api { code, .. }) if code == "NoSuchCORSConfiguration" => {
                return Ok(Vec::new())
            }
            Err(e) => return Err(e),
        };
    ...
    /// `DELETE /{bucket}/?cors`(OSS 不接受没有任何 `CORSRule` 的配置)。
    ...
    pub async fn get_bucket_website(&self, bucket: &str) -> Result<Option<WebsiteConfig>> {
        ...
        let resp = match check_status(self.http().execute(request).await?).await {
            Ok(resp) => resp,
            Err(OssError::Api { code, .. }) if code == "NoSuchWebsiteConfiguration" => {
                return Ok(None)
            }
            Err(e) => return Err(e),
        };
