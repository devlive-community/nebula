...
impl CosClient {
    ...
    /// 读取一个 bucket 的 CORS 规则:`GET /?cors`。未配置时服务端返回
    ...
    pub async fn get_bucket_cors(&self, bucket: &str) -> Result<Vec<CorsRule>> {
        ...
        let resp = match check_status(self.http().execute(request).await?).await {
            Ok(resp) => resp,
            Err(CosError::Api { code, .. }) if code == "NoSuchCORSConfiguration" => {
                return Ok(Vec::new())
            }
            Err(e) => return Err(e),
        };
    ...
    /// 设置一个 bucket 的 CORS 规则(整套替换):`PUT /?cors`。空列表时改发
    /// `DELETE /?cors`(COS 不接受没有任何 `CORSRule` 的配置)。
    ...
    /// 读取一个 bucket 的静态网站托管配置:`GET /?website`。未配置时,有的服务端返回
    /// `NoSuchWebsiteConfiguration` 错误,有的直接 200 返回一个没有 `IndexDocument` 的
    /// 空 `<WebsiteConfiguration/>`——两种情况都视为 `None`。
    pub async fn get_bucket_website(&self, bucket: &str) -> Result<Option<WebsiteConfig>> {
        ...
        let resp = match check_status(self.http().execute(request).await?).await {
            Ok(resp) => resp,
            Err(CosError::Api { code, .. }) if code == "NoSuchWebsiteConfiguration" => {
                return Ok(None)
            }
            Err(e) => return Err(e),
        };
