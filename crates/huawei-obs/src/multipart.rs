//! 分片上传:大文件分块上传,子资源签名(`uploads` / `uploadId` / `partNumber`)。
//!
//! 低层四步:[`ObsClient::initiate_multipart_upload`] → 多次
//! [`ObsClient::upload_part`] → [`ObsClient::complete_multipart_upload`],出错时
...
impl ObsClient {
    ...
    pub async fn upload_part(
        ...
        let etag = resp
            ...
            .map(|s| s.to_string())
            .ok_or_else(|| {
                ObsError::Core(cloud_core::CoreError::InvalidResponse(
    ...
    pub async fn get_object_tags(&self, bucket: &str, key: &str) -> Result<Vec<(String, String)>> {
        ...
        let resp = match check_status(self.http().execute(request).await?).await {
            Ok(resp) => resp,
            // 对象尚无标签时 OBS 返回 404 NoSuchTagSet(而非空 TagSet),视为空标签集。
            Err(ObsError::Api { code, .. }) if code == "NoSuchTagSet" => return Ok(Vec::new()),
            Err(e) => return Err(e),
        };
        let body = resp.text().await.map_err(cloud_core::CoreError::from)?;
        parse_tagging(&body)
...
mod tests {
    ...
    fn initiate_request_signs_uploads_subresource() {
        ...
        assert_eq!(
            req.url().as_str(),
            "https://examplebucket.obs.cn-north-4.myhuaweicloud.com/big.bin?uploads"
        );
        let sts = sign::string_to_sign("POST", "", "", date, "", "/examplebucket/big.bin?uploads");
        assert_eq!(
    ...
    fn tagging_request_signs_the_tagging_subresource() {
        ...
        // 且对象名按编码后路径签名(OBS 遵循 S3 V2:未解码的 Request-URI 路径)。
        let client = test_client();
        let date = "Thu, 17 Nov 2005 18:49:58 GMT";
        let req = client
            .build_part_request(
